...
// Added by Sindre
// slave address: 0x53, register_address: ? 
static uint16_t read_reg(uint8_t register_address, uint8_t data_returnValue) // should be reg. addr, data ptr. (to be returned after), size
{
		uint16_t rd;
		ret_code_t ret;
		uint8_t buff[2];
    ...
    return rd;	//rd; Dividing by 100 to see if the numbers sent are too large
		//return 65535; 
}

// Added by Sindre
static uint32_t write_reg(uint8_t register_address, uint8_t data_to_write, uint8_t size) // register, data, size
{
...
/** Added by Sindre from TWI Master example
